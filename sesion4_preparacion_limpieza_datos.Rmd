...
2. Determine si hay filas duplicadas. 

```{r}
filas_duplicadas_hospital_ <- datos_hospital |>  
  duplicated() |>  
  sum()

if (filas_duplicadas_hospital_ > 0) {
  print(paste("Hay", filas_duplicadas_hospital_, "filas duplicadas."))
} else {
...
```{r}
filas_vacias_eliminadas <- datos_hospital |> 
  drop_na() |> 
  nrow() - nrow(datos_hospital)

columnas_eliminadas <- datos_hospital |> 
  select_if(~ any(is.na(.))) |> 
  names()

print(paste("Se eliminaron", filas_vacias_eliminadas, "filas vacías."))
print(paste("Se eliminaron las siguientes columnas:", paste(columnas_eliminadas, collapse = ", ")))
```
...
datos_hospital <- datos_hospital |> 
  mutate(chills = ifelse(is.na(chills) | chills == "Yes", "Sí", chills),
         cough = ifelse(is.na(cough) | cough == "Yes", "Sí", cough),
         aches = ifelse(is.na(aches) | aches == "Yes", "Sí", aches),
         vomit = ifelse(is.na(vomit) | vomit == "Yes", "Sí", vomit))
```
...
```{r}
datos_hospital <- datos_hospital |> 
  select(-time_admission)
```

```{r}
datos_hospital <- datos_hospital |> 
  mutate(hospital = replace(hospital, is.na(hospital), "Sin información"))
```
